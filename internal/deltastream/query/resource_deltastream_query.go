...
func (d *QueryResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"queryID": schema.StringAttribute{
...
func (d *QueryResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if !util.ArrayContains([]string{kind}, []string{"INSERT_INTO"}) {
		resp.Diagnostics.AddError("planning error", "invalid query type: "+kind)
		return
	}

	statementPlan := statementPlan{}
	if err := json.Unmarshal([]byte(descJson), &statementPlan); err != nil {
		resp.Diagnostics.AddError("failed to parse query plan", err.Error())
		return
	}

	if statementPlan.Ddl != nil {
		resp.Diagnostics.AddError("planning error", "invalid query plan")
		return
	}

	if strings.TrimSpace(query.SinkRelation.ValueString()) != statementPlan.Sink.Fqn {
		resp.Diagnostics.AddError("planning error", "sink relation mismatch")
		return
	}
	...
	for _, source := range statementPlan.Sources {
		...
		for _, sourceRelation := range sourceRelations {
			if sourceRelation == source.Fqn {
				found = true
				break
			}
		}
		if !found {
			resp.Diagnostics.AddError("planning error", fmt.Sprintf("query uses source relation %s but it is not specified as a source on the resource", source.Fqn))
			return
		}
	}
	...
	if err := retry.Do(ctx, retry.WithMaxDuration(time.Minute*5, retry.NewExponential(time.Second)), func(ctx context.Context) (err error) {
		...
		if query.State.ValueString() != "errored" {
			return fmt.Errorf("query creation failed")
		}

		return nil
	}); err != nil {
		if _, derr := conn.ExecContext(ctx, fmt.Sprintf(`TERMIANTE QUERY %s;`, query.QueryID.ValueString())); derr != nil {
...
func (d *QueryResource) updateComputed(ctx context.Context, conn *sql.Conn, rel QueryResourceData) (QueryResourceData, error) {
	rows, err := conn.QueryContext(ctx, `LIST QUERIES;`)
	if err != nil {
		return rel, err
	}
	defer rows.Close()
...
func (d *QueryResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	resp.Diagnostics.AddError("invalid update", "query properties cannot be changed")

	currentQuery, err = d.updateComputed(ctx, conn, currentQuery)
	if err != nil {
